/**
 * 
 */
package defaultmethods;

import java.util.ArrayList;
import java.util.List;
import java.util.function.Predicate;

import defaultmethods.interfaces.PersonInterface;
import defaultmethods.model.Person;

/**
 * @author Manav
 *
 */
public class UseStaticMethod {

	/**
	 * @param args
	 */
	public static void main(String[] args) {

		List<Person> people = new ArrayList<Person>();
		people.add(new Person("Joe", 48));
		people.add(new Person("Mary", 30));
		people.add(new Person("Mike", 73));
		people.add(new Person("Watson", 65));

		Predicate<Person> pred = (p) -> p.getAge() >= 65;

		displayPeople(people, pred);
	}

	private static void displayPeople(List<Person> people, Predicate<Person> pred) {

		System.out.println("Selected:");
		people.forEach(p -> {
			if (pred.test(p)) {
				/*
				 * String info = p.getName() + " (" + p.getAge() + ")";
				 * System.out.println(info);
				 */

				// Using the default method
				//System.out.println("Printing info using default method :");
				System.out.println(p.getPersonInfo());
				
				// Using the static method
				//System.out.println("Printing info using static method : ");
				String personInfo = PersonInterface.getPersonInformation(p);
				System.out.println(personInfo);
			}
		});
	}

}
